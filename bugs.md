...
- Resolved: portable hot-update installation failed after download because the generated PowerShell script declared `$Pid`, which conflicts with PowerShell's read-only built-in `$PID` variable. The installer now uses `$AppPid`.
- Resolved: users with a CurseForge export ZIP/folder had to manually extract and select `overrides/` when no CurseForge API key was configured. No-baseline mode now imports the overrides payload directly and labels the target as overrides-only.
- Resolved: baseline apply could report `Update completed` without a post-write proof step. The apply path now verifies written, renamed, removed, and merged paths before state is written.
- Resolved: after a successful baseline apply, the UI could still show the same old-pack-to-new-pack raw diff as pending file changes. The update page now shows actionable plan files and refreshes the plan after apply.
- Resolved: operation history showed `0 files` for add-only operations because it counted backed-up old files instead of the durable operation file list.
